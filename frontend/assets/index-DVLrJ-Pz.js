import { r as s, q as x, j as e, f as h } from "./index-Bdf4ZDgc.js"; import { A as N, a as v } from "./AccountSection-Cnj__jkP.js"; import { P } from "./PageMeta-CaEzgttn.js"; function R() {
  const [a, y] = s.useState(null), [l, f] = s.useState([]), [m, b] = s.useState(0), [g, u] = s.useState(!0), [j, d] = s.useState(!0), [o, c] = s.useState(1), r = 10; s.useEffect(() => { x.getRoyalty().then(t => { t.data?.success && y(t.data.data) }).catch(console.error).finally(() => u(!1)) }, []), s.useEffect(() => { d(!0); const t = (o - 1) * r; x.getRoyaltyTransactions({ limit: r, offset: t }).then(p => { if (p.data?.success) { const i = p.data.data; f(i.rows ?? i.transactions ?? []), b(i.total ?? 0) } }).catch(console.error).finally(() => d(!1)) }, [o]); const n = Math.ceil(m / r); function w(t) { try { return new Date(t).toLocaleDateString("en-MY", { year: "numeric", month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" }) } catch { return t } } return e.jsx(N, {
