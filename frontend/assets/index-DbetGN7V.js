import { n as v, r as n, j as e, q as S } from "./index-DoE3uCvc.js"; import { A as w, a as P } from "./AccountSection-Bs0qKq1Z.js"; import { s as C, M as u, f as k, i as E, a as A, t as D } from "./malaysiaPhone-B0OpccwC.js"; import { P as M } from "./PageMeta-D_VAME4y.js"; function R() {
  const { user: c, setUser: g } = v(), [d, h] = n.useState(c?.name ?? ""), [o, y] = n.useState(() => { const s = c?.phone ?? ""; if (!s) return ""; const t = C(s); return t.startsWith(u) ? "0" + t.slice(u.length) : t }), l = c?.email ?? "", a = l.startsWith("ph_") || l.includes("@2Deal.app"), [m, j] = n.useState(a ? "" : l), [p, f] = n.useState(!1), [i, r] = n.useState(null); async function N(s) { if (s.preventDefault(), !d.trim()) return r({ type: "error", text: "Full name is required." }); if (o.trim() && !E(o)) return r({ type: "error", text: A }); r(null), f(!0); try { const t = { name: d.trim(), phone: o.trim() ? D(o) : "" }; a && m.trim() && (t.email = m.trim()); const b = (await S.updateProfile(t)).data.data; b && g(b), r({ type: "success", text: "Profile updated successfully." }) } catch (t) { const x = t?.response?.data?.message; r({ type: "error", text: x ?? "Failed to save changes. Please try again." }) } finally { f(!1) } } return e.jsx(w, {
...
} const Y = () => e.jsxs(e.Fragment, { children: [e.jsx(M, { title: "Setting | 2Deal - Incense Sticks, Soaps & Food Products Store", description: "2Deal - Incense Sticks, Soaps & Food Products Store" }), e.jsx(P, {}), e.jsx(R, {})] }); export { Y as default };
