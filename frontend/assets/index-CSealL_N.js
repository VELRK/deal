import{j as e,L as g,D as E,n as Z,r as c,s as ee,q as Y,v as te,w as L,f as s,p as O,A as oe,a as ae}from"./index-BHjvbaJH.js";import{a as re,s as D,d as v,r as se,b as ne,c as ce,M as ie}from"./MayBe-DAeCFku4.js";import{s as le}from"./shop-u_YYlLG-.js";import{P as de}from"./PageMeta-0jNrjtK6.js";import"./ProductCard-vDfFfoZC.js";import"./WishlistButton-0kmWpHWe.js";import"./productViewStore-lvNxlgFm.js";import"./TfSwiper-7Bdlkt1s.js";import"./shop-product-zLJqa8kw.js";function pe(){return e.jsx("section",{className:"section-page-title text-center flat-spacing-2 pb-0",children:e.jsx("div",{className:"container",children:e.jsxs("div",{className:"main-page-title",children:[e.jsxs("div",{className:"breadcrumbs",children:[e.jsx(g,{to:"/",className:"text-caption-01 cl-text-3 link",children:"Home"}),e.jsx("i",{className:"icon icon-CaretRightThin cl-text-3"}),e.jsx("p",{className:"text-caption-01",children:"Shopping Cart"})]}),e.jsx("h3",{children:"Shopping Cart"})]})})})}function xe(){const i=E(t=>t.cartProducts),o=E(t=>t.updateQuantity),n=E(t=>t.totalPrice),{isLoggedIn:p}=Z(),[w,N]=c.useState(""),[y,m]=c.useState(""),[k,j]=c.useState(0),[T,f]=c.useState(""),[q,z]=c.useState(!1),[l,M]=c.useState(null),[x,I]=c.useState(!1),[K,G]=c.useState(50),[C]=c.useState(100);c.useMemo(()=>{ee.get().then(t=>{if(t.data.success&&t.data.data){const r=t.data.data;typeof r.shipping_charge=="number"&&G(r.shipping_charge)}}).catch(t=>console.error("Failed to load settings",t))},[]),c.useEffect(()=>{if(!p||n<=0)return;const t=re();if(t){m(t.code),j(t.discount);return}const r=sessionStorage.getItem("sk_affiliate_ref");if(!r)return;let a=!1;return z(!0),Y.apply({code:r,order_amount:n}).then(u=>{if(a)return;const d=u.data;d.success&&d.data?(m(d.data.code),j(d.data.discount),D({code:d.data.code,discount:d.data.discount})):f(d.message??"Invalid affiliate promo code.")}).catch(u=>{if(a)return;const d=u?.response?.data?.message;f(d??"Could not apply affiliate promo code.")}).finally(()=>{a||z(!1)}),()=>{a=!0}},[p,n]),c.useEffect(()=>{if(!p){M(null);return}let t=!1;const r=a=>{t||(M(a),a&&!a.can_redeem&&x&&(I(!1),v(!1)))};return te.getRoyalty().then(a=>{const u=a.data?.data??null;if(u){r(u);return}return L.get().then(d=>{r(d.data?.data?.summary?.royalty??null)})}).catch(()=>{L.get().then(a=>r(a.data?.data?.summary?.royalty??null)).catch(()=>{t||M(null)})}),()=>{t=!0}},[p,i.length,n]);const U=t=>{I(t),v(t)};c.useEffect(()=>{i.length===0&&x&&(I(!1),v(!1))},[i.length,x]);const F=async()=>{const t=w.trim().toUpperCase();if(t){if(!p){f("Please login to apply a promo code.");return}z(!0),f("");try{const a=(await Y.apply({code:t,order_amount:n})).data;a.success&&a.data?(m(a.data.code),j(a.data.discount),N(""),D({code:a.data.code,discount:a.data.discount})):f(a.message??"Invalid promo code.")}catch(r){const a=r?.response?.data?.message;f(a??"Invalid or expired promo code.")}finally{z(!1)}}},J=()=>{m(""),j(0),f(""),D(null)},A=k,S=n<=0||n>=C?0:K,P=Math.max(0,n-A)+S,V=!!l&&l.enabled!==!1&&(!!l.show_on_cart||!!l.can_redeem||Number(l.points)>0),h=V&&!!l.can_redeem,B=se(l),_=h?"":ne(l),Q=h&&P>0,b=x&&Q?Math.min(Number(l?.balance_rm||0),P):0,$=Math.max(0,P-b),R=Math.max(0,C-n),W=(t,r,a)=>{oe(t,r,a)},X=(t,r,a,u)=>{if(r<1){W(t,a,u);return}o(t,r,a),L.update({product_id:Number(t),quantity:r,...a!=null?{variant_id:a}:{}}).catch(()=>{})};return e.jsxs(e.Fragment,{children:[e.jsx("style",{children:`
