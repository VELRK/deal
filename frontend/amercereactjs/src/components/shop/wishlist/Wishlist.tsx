...
function WishlistTableRow({
  ...
  const variants = product.unitVariants ?? [];
  const isOutOfStock = variants.length > 0
    ? variants.every((v) => Number(v.stock ?? 0) <= 0)
    : !!product.isStockOut || Number(product.stock ?? 0) <= 0;
    ...
    <div className="classic-wishlist-item">
      ...
      <div className="wishlist-item-details">
        ...
        <div className="wishlist-item-stock">
          <span className="status-dot" style={isOutOfStock ? { background: "#dc2626" } : undefined}></span>
