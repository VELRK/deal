...
export function ProductCardClassicView() {
  ...
  return (
    ...
    >
      ...
      <div className={`card-product_info p-2 d-flex flex-column flex-grow-1 ${infoClassName}`.trim()}>
        <Link
          to={`/product-detail/${product.id}`}
          className="name-product text-dark flex-grow-1 text-decoration-none"
          style={{ fontFamily: "Georgia, serif", fontSize: "14px", lineHeight: "1.3", color: "#333", marginBottom: "4px" }}
        >
          {product.name}
        </Link>
        ...
        <div className="d-flex align-items-center mb-2 gap-2 flex-wrap">
          ...
          <span className="price-new" style={{ color: "#5a2d26", fontSize: "16px", fontWeight: "500" }}>
